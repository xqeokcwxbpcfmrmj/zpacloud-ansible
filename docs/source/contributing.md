...
Report bugs at https://github.com/willguibr/zpacloud/issues.
...
https://github.com/willguibr/zpacloud/issues.
...
Ready to contribute some code? Here's how to set up `zpacloud` for local development.
...
2. Fork the `zpacloud` repo on GitHub.
...
```
$ mkdir -p ansible_collections/zpacloud
$ git clone https://github.com/your-username/zpa-ansible.git ansible_collections/willguibr/zpacloud
```
...
4. Create a playbooks directory, and add our top level directory to `ansible.cfg`:
   ...
   `ansible_collections/willguibr/zpa-ansible` as the collection
   `willguibr.zpacloud` without us having to build and install the collection each
   time!
